...
def process_notion_db(results):
  ...
  notion_df = pd.DataFrame(columns=[
    "date", "no_drinks", "no_binge_drinking", "journal_entry", "read_15_min",
  ...
  for result in results["results"]:
    row = {
      ...
      "no_binge_drinking":
      result["properties"]["No Binge Drinking"]["checkbox"],
...
def get_all_notion_data(api_key):
  # API Key and DB ID
...
def query_notion_table(db="../health_data.db"):
  ...
  df = pd.DataFrame(results,
                    columns=[
                      'date', 'no_drinks', 'no_binge_drinking',
                      'journal_entry', 'read_15_min', 'morning_energy',
                      'health_rating', 'work_rating', 'life_rating'
                    ])
